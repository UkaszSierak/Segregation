...
def ExtractCoordinates(message: str)-> dict:
    coordinates = {}
    pattern = re.compile(r'[XYZABCL][1]?;-?\d+\.\d+')
...
def GetNewLocation(correction: dict, current_location: dict):
    new_location = current_location

    for item in correction:
        if item in new_location:
            new_location[item] = correction[item]

    return new_location

def PrepareLocation(location: dict):

    order = ['X', 'Y', 'Z', 'A', 'B', 'C', 'L1']
    result = ()
    for item in order:
        if item in location:
            result += (location[item],)
        else:
            print('Error, didnt find all values')
            result = ()
            break
    return result
